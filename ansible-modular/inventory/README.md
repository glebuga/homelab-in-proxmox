...
декларативный файл [`mapping.yml`](mapping.yml). **Чтобы добавить или
убрать машину, правьте только `mapping.yml` — код `terraform_inventory.py`
трогать не нужно.** Скрипт — универсальный движок, который читает эту
конфигурацию и строит inventory.

Поддерживаются два вида записей:

1. **Простой хост** (output — строка/IP), с фиксированным именем:

   ```yaml
   nginx_ip:
     host: nginx            # имя хоста в Ansible
     groups: [nginx_nodes]  # группы
     bootstrap: true        # добавить bootstrap_user=root
   ```

2. **VM-словарь** (output — dict по `vm_id`, имя собирается динамически):

   ```yaml
   k3s_vm_ips:
     host_prefix: k3s-      # имя = prefix + vm_id  (k3s-205, k3s-206)
     groups: [kuber]
     bootstrap: false
   ```

Опционально можно переопределить пользователя подключения через `user:`
(полезно для будущих CentOS-хостов с другим admin-пользователем).
...
terraform.tfvars (ip_address)
   ...
   -> terraform_inventory.py (читает mapping.yml, ставит ansible_host = IP)
   -> группы Ansible
```
...
правок в 4 файлах Terraform + 1 в `mapping.yml` инвентаря.
...
6. `ansible-modular/inventory/mapping.yml` — добавь одну запись (скрипт
   `terraform_inventory.py` править **не нужно**, он data-driven):
   ```yaml
   monitoring_ip:
     host: monitoring
